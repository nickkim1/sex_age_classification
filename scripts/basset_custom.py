...
class CustomDataset(Dataset): 
    def __init__(self, dataset, n_samples): 
        ...
        for i in range(self.n_samples): # n_samples is the number of rows, 13854
            ...
            for j in range(600): 
                # one hot encode sequences as: A C T G (top -> bottom)
                idx_to_mark = 0
...
learning_rate = 0.0001 # arbitrary choice for the lr 
...
def train_model():
    ...
    train_log = {'training_loss_per_epoch':[], 'training_accuracy_per_epoch':[]} # keep track of params 
    ...
    PATH = '../saved_models/basset_params.pth'
...
test_log = test_model('../saved_models/basset_params.pth')
...
def plot_loss(id, num_epochs, train_loss, test_loss):
    ...
    a.legend()
    # plt.show() 
    plt.savefig(f'../viz/loss_curves_{id}')
...
def plot_accuracy(id, num_epochs, train_accuracy, test_accuracy): 
    ...
    a.legend()
    # plt.show() 
    plt.savefig(f'../viz/accuracy_curves_{id}')


# call the loss function plotter 
plot_loss("01", np.linspace(1, num_epochs, num=num_epochs).astype(int), train_log['training_loss_per_epoch'], test_log['testing_loss_per_epoch'])
plot_accuracy("01", np.linspace(1, num_epochs, num=num_epochs).astype(int), train_log['training_accuracy_per_epoch'], test_log['testing_accuracy_per_epoch'])